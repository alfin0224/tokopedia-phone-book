...
function ContactList() {
  ...
  useEffect(() => {
    ...
    setFavoriteContacts(savedContacts);
  }, []);
  ...
  const allContacts = [...data.contact].sort((a: Contact, b: Contact) =>
  ...
  const favoriteContactsToShow = favoriteContacts.filter(
    (contact: Contact) =>
      contact.first_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      contact.last_name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const allContactsToShow = allContactsExceptFavorites
    .filter(
      (contact: Contact) =>
        contact.first_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        contact.last_name.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const allContactsToShowMobile = allContactsExceptFavorites.filter(
    (contact: Contact) =>
      contact.first_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      contact.last_name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const totalPagesForAllContacts = Math.ceil(
    allContactsExceptFavorites.length / pageSize
  );
  ...
  const addToFavorites = (contact: Contact) => {
    if (!favoriteContacts.includes(contact)) {
      ...
      saveContactsToLocalStorage(updatedFavoriteContacts);
    }
  };
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            {showMoreId === contact.id ? (
                              <FaExpand style={{ color: "#329c37" }} />
                            ) : (
                              <FaBars style={{ color: "#329c37" }} />
                            )}
                          </button>
                        </div>
                      </ContactCard>
